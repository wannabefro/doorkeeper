...
module Doorkeeper
  describe 'Application (aka Client)' do
    let(:new_application) { build(:application) }
    ...
    describe 'destroy related models on cascade' do
      ...
      it 'should destroy its access grants' do
        create(:access_grant, application: new_application)
        expect { new_application.destroy }.to change { Doorkeeper::AccessGrant.count }.by(-1)
      end

      it 'should destroy its access tokens' do
        create(:access_token, application: new_application)
        create(:access_token, application: new_application, revoked_at: Time.now.utc)
    ...
    describe :authorized_for do
      ...
      it "returns only application for a specific resource owner" do
        create(:access_token, resource_owner_id: resource_owner.id + 1)
        token = create(:access_token, resource_owner_id: resource_owner.id)
        Doorkeeper.client.authorized_for(resource_owner).should == [token.application]
      end

      it "excludes revoked tokens" do
        create(:access_token, resource_owner_id: resource_owner.id, revoked_at: 2.days.ago)
        Doorkeeper.client.authorized_for(resource_owner).should be_empty
      end

      it "returns all applications that have been authorized" do
        token1 = create(:access_token, resource_owner_id: resource_owner.id)
        token2 = create(:access_token, resource_owner_id: resource_owner.id)
        Doorkeeper.client.authorized_for(resource_owner).should == [token1.application, token2.application]
      end

      it "returns only one application even if it has been authorized twice" do
        application = create(:application)
        create(:access_token, resource_owner_id: resource_owner.id, application: application)
        create(:access_token, resource_owner_id: resource_owner.id, application: application)
